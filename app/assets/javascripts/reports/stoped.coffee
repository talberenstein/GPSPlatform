...
aux = 0
$(document).on 'ready', ->
  $('#search_stop_report').on 'click', ->
    do_stop_report()
  $("#report_tab_playback").hide()
  $("#report_tab_speed").hide()
  $("#report_tab_stop").show()
  ...
  stop_reports_table = $('#stop_reports_table').DataTable(
    data: []
    bLengthChange: false
    processing: true
    serverSide: false
    ...
    "fnRowCallback": (nRow, aData, iDisplayIndex) ->
      $('td:eq(6)', nRow).html '<button type="button" class="show_path_information btn waves-effect waves-light" data-id="' + aData.id + '">  <i class="material-icons">mode_edit</i></button>'
  )
...
do_stop_report = ->
  c_date_i = $('#report_i_date_stop').val()
  c_date_f = $('#report_f_date_stop').val()
  c_time_i = $('#report_i_time_stop').val()
  c_time_f = $('#report_f_time_stop').val()
  console.log c_time_i + ' to ' + c_time_f
  Materialize.toast('Consultando Base de Datos', (1000) * 60 * 5)

  stop_limit = $('#report_f_stop').val()

  $.ajax '/information/report_stops',
    type: 'GET'
    data: {
      from_date: c_date_i
      from_time: c_time_i
    ...
    success: (data, textStatus, jqXHR) ->
      ...
      setTimeout ( ->
        $('.show_path_information').on 'click', ->
          tr = $(this).closest('tr')
          row = stop_reports_table.row(tr)
          draw_map(row.data())
      ), 500

      stop_reports_table.clear().draw()
      stop_reports_table.rows.add(data).draw()
      console.log data

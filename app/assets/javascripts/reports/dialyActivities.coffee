...
dialyActivities_reports_table = null
...
aux = 0
$(document).on 'ready', ->
  $('#search_dialyActivities_report').on 'click', ->
    do_dialyActivities_report()
  $("#report_tab_playback").hide()
  $("#report_tab_speed").hide()
  $("#report_tab_stop").hide()
  $("#report_tab_dialyActivities").show()
  ...
  dialyActivities_reports_table = $('#dialyActivities_reports_table').DataTable(
    data: []
    bLengthChange: false
    processing: true
    serverSide: false
    ...
    columns: [
      ...
      {"data": "direction"},
      {"data": "position_type"},
      {"data": "position_antiquity"},
      {"data": "odometer"},
      {"data": "seen_satellites"},
      {"data": "gps_valid"},
      {"data": "to_graph"},
    ],
    "fnRowCallback": (nRow, aData, iDisplayIndex) ->
      $('td:eq(14)', nRow).html '<button type="button" class="show_path_information btn waves-effect waves-light" data-id="' + aData.id + '">  <i class="material-icons">mode_edit</i></button>'
  )


do_dialyActivities_report = ->
  c_date_i = $('#report_i_date_dialy').val()
  c_date_f = $('#report_f_date_dialy').val()
  c_time_i = $('#report_i_time_dialy').val()
  c_time_f = $('#report_f_time_dialy').val()
  console.log c_time_i+' to '+c_time_f
  Materialize.toast('Consultando Base de Datos', (1000) * 60 * 5)
  $.ajax '/information/report_dialyActivities',
    type: 'GET'
    data: {
      from_date: c_date_i
      from_time: c_time_i
      to_date: c_date_f
      to_time: c_time_f
      imeis: $("#dialyActivities_devices").val()
    }
    ...
    success: (data, textStatus, jqXHR) ->
      ...
      setTimeout ( ->
        $('.show_path_information').on 'click', ->
          tr = $(this).closest('tr')
          row = dialyActivities_reports_table.row(tr)
          draw_map(row.data())
      ), 500

      dialyActivities_reports_table.clear().draw()
      dialyActivities_reports_table.rows.add(data).draw()
      console.log data
